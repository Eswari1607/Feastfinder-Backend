...
router.post("/submit-review", async (req, res) => {
  try {
    ...
    if (
      ...
      return res
        .status(400)
        .json({ message: "Invalid hotelId or userId format" });
    }
...
router.put("/edit-review/:reviewId", async (req, res) => {
  try {
    ...
    // Ensure the user who is trying to edit the review is the same user who created it
    if (existingReview.userId.toString() !== req.body.userId) {
      return res
        .status(403)
