...
router.get("/bookings/:userId", async (req, res) => {
  ...
  try {
    ...
    if (bookings.length === 0) {
      return res
        .status(404)
        .json({ message: "No user found or no bookings available." });
    }
...
router.put("/editBooking/:bookingId", async (req, res) => {
  const { bookingId } = req.params;
  const { selectedDate, selectedTime, selectedSeats } = req.body;
  ...
  try {
    ...
    if (selectedTime) booking.selectedTime = selectedTime;
